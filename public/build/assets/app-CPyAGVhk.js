...
`).filter(e=>e.trim()).map(e=>`<div class="sf"><i class="fas fa-check-circle"></i>${e}</div>`).join(``));let s=r(`svcFq`);s&&(s.innerHTML=(e.g(`faqs`)||[]).slice(0,3).map(e=>`
        ...
        `:n.innerHTML=i.map(e=>{if(e.svc===`video`||e.svc===`فيديو`||e.text&&(e.text.includes(`youtube.com`)||e.text.includes(`youtu.be`)||e.text.endsWith(`.mp4`))){let t=``,n=``;return(e.text.includes(`youtube.com`)||e.text.includes(`youtu.be`))&&(e.text.includes(`youtube.com/embed/`)?n=e.text.split(`embed/`)[1].split(`?`)[0]:e.text.includes(`youtube.com/watch?v=`)?n=e.text.split(`watch?v=`)[1].split(`&`)[0]:e.text.includes(`youtu.be/`)&&(n=e.text.split(`youtu.be/`)[1].split(`?`)[0]),n&&(t=`https://img.youtube.com/vi/${n}/hqdefault.jpg`)),`
            ...
            `}).join(``)}function I(e){console.log(`%c[Video Player] openVid triggered for URL: "${e}"`,`color: #e07b0f; font-weight: bold; font-size: 13px;`);let t=r(`vidModal`),n=r(`vidBody`);if(!t){console.error(`%c[Video Player] ERROR: HTML element with ID 'vidModal' was not found in the DOM!`,`color: red; font-weight: bold;`);return}if(!n){console.error(`%c[Video Player] ERROR: HTML element with ID 'vidBody' was not found in the DOM!`,`color: red; font-weight: bold;`);return}console.log(`[Video Player] vidModal and vidBody successfully located.`);let i=``;if((e.includes(`youtube.com`)||e.includes(`youtu.be`))&&(e.includes(`youtube.com/embed/`)?i=e.split(`embed/`)[1].split(`?`)[0]:e.includes(`youtube.com/watch?v=`)?i=e.split(`watch?v=`)[1].split(`&`)[0]:e.includes(`youtu.be/`)&&(i=e.split(`youtu.be/`)[1].split(`?`)[0])),i){console.log(`%c[Video Player] YouTube Video detected. ID: ${i}`,`color: #ff0000; font-weight: bold;`),n.innerHTML=`<iframe src="https://www.youtube.com/embed/${i}?autoplay=1&mute=0&controls=1&rel=0" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen style="width:100%; height:100%; border:none;"></iframe>`;let e=n.querySelector(`iframe`);e.onload=function(){console.log(`%c[Video Player] YouTube IFrame loaded successfully.`,`color: green; font-weight: bold;`)}}else{console.log(`[Video Player] Native MP4/WebM video detected.`);let t=e;if(e.startsWith(`http`)||(t=e.split(`/`).map(function(e){return e&&encodeURIComponent(e)}).join(`/`),console.log(`[Video Player] Encoded native video path to: "${t}"`)),(window.location.hostname===`localhost`||window.location.hostname===`127.0.0.1`||window.location.hostname.startsWith(`192.168.`))&&!e.startsWith(`http`)){let n=e.replace(/^\/+/,``);t=`/video-stream?path=${encodeURIComponent(n)}`,console.log(`[Video Player] Local dev detected. Streaming video via: "${t}"`)}n.innerHTML=`<video src="${t}" controls playsinline style="width:100%; height:100%; object-fit:contain;" onclick="event.stopPropagation()"></video>`;let r=n.querySelector(`video`);r&&(console.log(`[Video Player] Created native <video> element. Setting up diagnostics event listeners...`),r.addEventListener(`loadstart`,()=>{console.log(`%c[Video Player] [Event: loadstart] Video is beginning to load: "${t}"`,`color: #3182ce;`)}),r.addEventListener(`loadedmetadata`,()=>{console.log(`%c[Video Player] [Event: loadedmetadata] Video metadata successfully loaded. Duration: ${r.duration.toFixed(2)}s, Dimensions: ${r.videoWidth}x${r.videoHeight}`,`color: #2b6cb0; font-weight: bold;`)}),r.addEventListener(`canplay`,()=>{console.log(`%c[Video Player] [Event: canplay] Enough data is buffered. Video is ready to start playing.`,`color: #2f855a; font-weight: bold;`)}),r.addEventListener(`playing`,()=>{console.log(`%c[Video Player] [Event: playing] Video playback has started or resumed successfully.`,`color: green; font-weight: bold;`)}),r.addEventListener(`waiting`,()=>{console.warn(`[Video Player] [Event: waiting] Video playback stopped due to buffering...`)}),r.addEventListener(`error`,e=>{let t=r.error,n=`Unknown Media Error`;if(t)switch(t.code){case t.MEDIA_ERR_ABORTED:n=`Fetching process aborted by user.`;break;case t.MEDIA_ERR_NETWORK:n=`A network error occurred while fetching the video. Please check your internet connection.`;break;case t.MEDIA_ERR_DECODE:n=`An error occurred while decoding the video (corrupted file or unsupported codec format).`;break;case t.MEDIA_ERR_SRC_NOT_SUPPORTED:n=`The video file could not be loaded. Either the path/file is not found (404 Not Found), or your browser doesn't support this specific format/codec.`;break}console.error(`%c[Video Player] [Event: error] FAILED to play video: ${n}`,`color: red; font-weight: bold; font-size: 12px;`,t)}),console.log(`[Video Player] Requesting browser to play video...`),r.play().then(()=>{console.log(`%c[Video Player] Video play request accepted by browser.`,`color: green;`)}).catch(e=>{console.warn(`%c[Video Player] Autoplay with sound blocked or interrupted. The user can press play manually.`,`color: orange;`,e.message)}))}t.style.display=`flex`,console.log(`[Video Player] Modal display style changed to 'flex'.`)}function L(){let e=r(`vidModal`),t=r(`vidBody`);e&&(e.style.display=`none`),t&&(t.innerHTML=``)}window.openVid=I,window.closeVid=L,document.dispatchEvent(new Event(`app:ready`));function _e(e,t){P=e,document.querySelectorAll(`.test-f .tf`).forEach(e=>{e.classList.remove(`act`),e.style.background=`transparent`,e.style.color=`var(--nv)`,e.style.borderColor=`var(--sl2)`}),t&&(t.classList.add(`act`),t.style.background=`rgba(224, 123, 15, 0.06)`,t.style.color=`var(--am)`,t.style.borderColor=`var(--am)`),F()}window.fTest=_e;function ve(e){r(e)?.classList.toggle(`open`)}function R(e){let t=e.img&&e.img.startsWith(`http`);if(e.cat===`فيديو`||e.cat===`video`||e.img&&(e.img.endsWith(`.mp4`)||e.img.includes(`youtube.com`)||e.img.includes(`youtu.be`))){let t=e.img,n=``;return e.img&&(e.img.includes(`youtube.com`)||e.img.includes(`youtu.be`))&&(e.img.includes(`youtube.com/embed/`)?n=e.img.split(`embed/`)[1].split(`?`)[0]:e.img.includes(`youtube.com/watch?v=`)?n=e.img.split(`watch?v=`)[1].split(`&`)[0]:e.img.includes(`youtu.be/`)&&(n=e.img.split(`youtu.be/`)[1].split(`?`)[0]),n&&(t=`https://img.youtube.com/vi/${n}/hqdefault.jpg`)),`
